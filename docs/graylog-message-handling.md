...
The chart's readiness probe is an HTTP check against this same endpoint, so forcing
...
> [!IMPORTANT]
> Once the pod leaves the endpoints, the Service can no longer reach it — so run the
...
> internally generated inputs keep writing, which is why step 1 is the step that actually
> lets the journal reach zero.
